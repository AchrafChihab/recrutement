    ...
    <div class="container">
        ...
        <div class="topbar-left">
            ...
            <a class="topbar-brand" href="{{ url('/') }}">
                <img src="{{ $global->logo_url }}" class="logo-inverse" alt="Digitalia solutions" />
            </a>
        </div>
            ...
            <div class="d-inline-flex ml-200 language-drop">
                <div class="dropdown btn btn-default" style="margin-left: 390px;">
                    <a href="#" class="dropdown-toggle text-capitalize" data-toggle="dropdown">
                        <i class="flag-icon @if(App::getLocale() == 'en') flag-icon-us @else  flag-icon-{{ App::getLocale() }} @endif"></i> {{ $languageSettings->where('language_code', App::getLocale())->first()->language_name ?? 'Language' }}
                    </a>
                    <div class="dropdown-menu" style="margin-left: 10px">
                        @forelse ($languageSettings as $language)
                            <a class="dropdown-item language-select @if(App::getLocale() == $language->language_code) active @endif" data-lang-code="{{ $language->language_code }}" href="javascript:;">
                                <span class="flag-icon @if($language->language_code == 'en') flag-icon-us @elseif($language->language_code == 'ar') flag-icon-ae @elseif($language->language_code == 'es-ar') flag-icon-ar @else flag-icon-{{ $language->language_code }} @endif"></span> {{ $language->language_name }}
                            </a>
                        @empty
                            <span class="dropdown-item">@lang('No languages available')</span>
                        @endforelse
                    </div>
                </div>
            </div>
...
<footer class="site-footer">
    <div class="container">
        <div class="text-center row">
            <div class="mb-10 col-12 col-lg-12">
            ...
            <div class="col-12 col-lg-12 fw-400">
                ...
                <p>&copy; {{ \Carbon\Carbon::today()->year }} @lang('app.by') Digitalia Solutions</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var languageItems = document.querySelectorAll('.language-select');

        languageItems.forEach(function (item) {
            item.addEventListener('click', function () {
                var selectedLang = this.getAttribute('data-lang-code');
                
                // Get the current URL
                var currentUrl = new URL(window.location.href);
                
                // Set or update the locale parameter
                currentUrl.searchParams.set('locale', selectedLang);
